...
x <-as.matrix(train_subset[, sapply(train_subset, is.numeric)])
x2 <-as.matrix(test_subset[, sapply(test_subset, is.numeric)])
...
The model's test error of 0.76 indicates poor performance on the test data, suggesting that PLS may not be suitable for this dataset due to its high error rate. This implies that the variables in the dataset are likely not highly correlated, making the assumption underlying PLS inappropriate. Instead of combining features, the dataset's variables should be analyzed individually to better capture their distinct contributions using Ridge regression.

# Final Conclusions

The model with the best MSE was Ridge with an error of 0.007210671. This seems appropriate given that ridge models perform well with data with high collinearity. This model also allows parameters to not be completely eliminated, but to have the degree of their effects modified using appropriate weights.
