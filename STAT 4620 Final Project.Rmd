...
(mselasso <- mean((test_subset$Metrics.Sales-pred.lasso)ˆ2))

#HERE: TEST AND TRAIN SUBSETS DONT HAVE THE SAME VARIABLES? SEEMS LIKE AN ISSUE
library(dplyr)
comparison <- tibble(
  Variable = union(colnames(x), colnames(x2)),
  In_df1 = Variable %in% colnames(x),
  In_df2 = Variable %in% colnames(x2)
)
comparison
```
Using a lambda of 0.01422756 (minimized), the test error of the ridge model was [INSERT].
